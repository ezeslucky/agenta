...
-- "one agenta Agent ↔ one Lark Bot"; re-installing on the same agent
...
-- Records that a Lark open_id (per-installation) maps to a agenta
-- user.
--
...
--   2. ON CONFLICT DO UPDATE is gated on `agenta_user_id` matching
...
INSERT INTO lark_user_binding (
    workspace_id, agenta_user_id, installation_id, lark_open_id, union_id
) VALUES (
...
ON CONFLICT (installation_id, lark_open_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE lark_user_binding.agenta_user_id = EXCLUDED.agenta_user_id
RETURNING *;

-- name: GetLarkUserBindingByOpenID :one
-- The inbound identity check. A row here means: this open_id maps to a
-- agenta user who IS currently a workspace member (the composite FK
