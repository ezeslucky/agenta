...
// the agenta CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `agenta issue create` failure — see PR
...
var (
	ErrCLIVersionMissing = errors.New("agenta CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("agenta CLI version is below required minimum")
)
