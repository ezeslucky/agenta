...
const (
	agentaCodexMcpBeginMarker = "# BEGIN agenta-managed mcp_servers (do not edit; regenerated by daemon)"
	agentaCodexMcpEndMarker   = "# END agenta-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agentaCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agentaCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = agentaCodexMcpBeginMarker + "\n" + agentaCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(agentaCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(agentaCodexMcpEndMarker)
	sb.WriteString("\n")
