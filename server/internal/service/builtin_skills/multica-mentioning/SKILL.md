...
This skill states WHAT a mention link does in the Agenta backend, traced to
...
- a person → `agenta workspace member list --output json` → use `user_id`
- an agent → `agenta agent list --output json` → use `id`
- a squad  → `agenta squad list --output json` → use `id`
...
Correct:
  1. `agenta workspace member list --output json`  → Alice's `user_id` = 7f3a…
