...
```bash
agenta agent get <agent-id> --output json      # full persisted agent record
agenta agent skills list <agent-id> --output json   # current skill bindings
agenta agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
`POST /api/agents` (`agenta agent create`). At task claim time the daemon
...
```bash
agenta agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
agenta agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`agenta agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`agenta agent env set`), which is owner/admin-only
...
```bash
agenta agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
agenta agent update <agent-id> --mcp-config-stdin --output json
agenta agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `agenta agent create` — inserts a new agent row.
- `agenta agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `agenta agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
