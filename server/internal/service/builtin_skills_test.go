...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The agenta- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "agenta-") {
				t.Errorf("skill name %q must carry the agenta- prefix", skill.Name)
			}
...
// must declare user-invocable:false and fence itself to the agenta CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "agenta-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(agenta *)" {
		t.Errorf("allowed-tools = %q, want Bash(agenta *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "agenta-working-on-issues")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "Bash(agenta *)") {
		t.Errorf("allowed-tools = %q, want access to the Agenta CLI", got)
	}
	...
	mustContain := []string{
		...
		"open or update a PR before posting the final Agenta issue comment",
	...
	mustNotContain := []string{
		...
		"agenta issue metadata list <issue-id> --output json",
		"agenta issue comment list <issue-id> --thread <trigger-comment-id>",
		"agenta issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "agenta-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustNotContain := []string{
		"agenta agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "agenta-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"agenta agent create --name <name> --runtime-id <runtime-id>",
		...
		"agenta agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"agenta agent skills list <agent-id> --output json",
		"agenta agent get <agent-id> --output json",
		"255",
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "agenta-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "agenta-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"agenta autopilot trigger-add <autopilot-id> --kind schedule",
		"agenta autopilot trigger <autopilot-id> --output json",
		"Do not run `trigger`",
		"webhook tokens",
		"{{date}}",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "agenta-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"agenta runtime list --output json",
...
func TestProjectsAndResourcesSkillCoversDurableContext(t *testing.T) {
	skill, ok := findSkill(t, "agenta-projects-and-resources")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"agenta project resource add <project-id> --type local_directory",
