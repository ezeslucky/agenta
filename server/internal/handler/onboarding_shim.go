...
const (
	onboardingAssistantName = "Agenta Helper"
	onboardingIssueTitle    = "Start here: learn Agenta with Agenta Helper"
	onboardingAgentTemplate = "agenta_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Agenta questions and runs CLI operations."
...
// agenta Helper agent created by this shim. Pre-v3 desktop submits a
...
// is removed.
const onboardingAssistantInstructions = `You are Agenta Helper, the built-in AI assistant for this Agenta workspace. Your role is to help any member use Agenta better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to Agenta.

This is your guided first run. Agenta Helper is assigned to this issue and will help you try the core workflow:

1. Read Agenta Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Agenta Helper when you want it to continue.
...
//
// Creates or reuses one "agenta Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Agenta.",
		"",
		"Agents need a runtime before they can execute work. You can still use Agenta as a lightweight project-management workspace while you install one.",
		"",
		"## Try Agenta first",
		"",
		...
		"",
		"Full guide: https://agenta.ai/docs/install-agent-runtime",
		"",
		...
		"",
		"When the runtime is connected, you can create Agenta Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Agenta。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Agenta 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://agenta.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Agenta Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
