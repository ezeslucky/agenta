...
// translates the prompt into a `agenta issue create` invocation in the
...
// the agent's `agenta issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the agenta CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See ezeslucky/agenta#1660.
