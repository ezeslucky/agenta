...
func TestBusinessSamplerCollectorEmitsExpectedMetrics(t *testing.T) {
	...
	wantSubstrings := []string{
		`agenta_active_users{window="5m"} 7`,
		`agenta_active_workspaces{window="5m"} 3`,
		`agenta_agent_task_queued{source="chat"} 5`,
		`agenta_agent_task_queued{source="issue"} 2`,
		// Zero series for sources that didn't appear in the snapshot.
		`agenta_agent_task_queued{source="autopilot"} 0`,
		`agenta_agent_task_queued{source="other"} 0`,
		`agenta_agent_task_running{runtime_mode="cloud",source="chat"} 3`,
		`agenta_agent_task_running{runtime_mode="local",source="issue"} 1`,
		`agenta_agent_task_stuck_total{source="issue"} 1`,
		`agenta_runtime_online{provider="claude",runtime_mode="local"} 4`,
		`agenta_runtime_online{provider="kiro",runtime_mode="cloud"} 2`,
		`agenta_runtime_heartbeat_age_seconds_count{runtime_mode="local"} 3`,
		`agenta_runtime_heartbeat_age_seconds_sum{runtime_mode="local"} 45`,
		`agenta_workspace_total 250`,
	}
	...
	for _, removed := range []string{
		`agenta_active_users{window="1h"}`,
		`agenta_active_users{window="24h"}`,
		`agenta_active_workspaces{window="1h"}`,
		`agenta_active_workspaces{window="24h"}`,
	} {
...
func TestBusinessSamplerSelfIntrospectionHistogramIsExposed(t *testing.T) {
	...
	NewHandler(registry).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/metrics", nil))
	body := rec.Body.String()
	if !strings.Contains(body, `agenta_business_sampler_query_seconds_count{name="active_users"} 1`) {
...
func TestBusinessSamplerCollectorBoundedCardinality(t *testing.T) {
	...
	if got := testutil.CollectAndCount(c, "agenta_agent_task_queued"); got != len(knownSourceLabels()) {
	...
	if got := testutil.CollectAndCount(c, "agenta_agent_task_running"); got != expectedRunning {
		t.Fatalf("agent_task_running series = %d, want %d", got, expectedRunning)
	}
	if got := testutil.CollectAndCount(c, "agenta_runtime_online"); got != 1 {
...
func TestBusinessSamplerCollectorDisabledWithoutOptions(t *testing.T) {
	...
	for _, forbidden := range []string{
		"agenta_active_users",
		"agenta_agent_task_queued",
		"agenta_runtime_online",
		"agenta_business_sampler_query_seconds",
	} {
