...
import (
	...
	"github.com/ezeslucky/agenta/server/internal/migrations"
	"github.com/ezeslucky/agenta/server/internal/taskusagebackfill"
)
...
func TestHook_DirectV034Upgrade(t *testing.T) {
	...
	if adminURL == "" {
		adminURL = "postgres://agenta:agenta@localhost:5432/agenta?sslmode=disable"
	}
	...
	tmpDB := fmt.Sprintf("agenta_v034_upgrade_%d", time.Now().UnixNano())
...
func TestHook_FreshDatabaseStampsWatermarkOnly(t *testing.T) {
	...
	tmpDB := fmt.Sprintf("agenta_v034_fresh_%d", time.Now().UnixNano())
	if err := createDatabase(ctx, adminURL, tmpDB); err != nil {
