...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "agenta login") {
		t.Fatalf("401 warning must tell the user to run 'agenta login', got: %s", out)
	}
}
...
// and the operator would never see a "run agenta login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "agenta login") {
		t.Fatalf("expected the actionable 'run agenta login' hint in the WARN, got: %s", out)
	}
}
