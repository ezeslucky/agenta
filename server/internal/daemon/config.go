...
import (
	...
	"github.com/ezeslucky/agenta/server/internal/cli"
)
...
const (
	...
	// hard ceiling for cost/resource control can set AGENTA_AGENT_TIMEOUT.
	...
	// Set AGENTA_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set AGENTA_AGENT_TOOL_WATCHDOG=0 to
...
// AGENTA_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // agenta CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/AGENTA_workspaces)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on agenta Cloud, false on self-host)
	...
	// written by `agenta runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("AGENTA_SERVER_URL", DefaultServerURL)
	...
	//
	//   - AGENTA_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	// users already get with AGENTA_OPENCLAW_PATH today. We achieve it with
	...
	// pinned to an explicit AGENTA_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned AGENTA_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("AGENTA_CLAUDE_PATH", "claude", "AGENTA_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("AGENTA_CODEX_PATH", "codex", "AGENTA_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("AGENTA_OPENCODE_PATH", "opencode", "AGENTA_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("AGENTA_OPENCLAW_PATH", "openclaw", "AGENTA_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("AGENTA_HERMES_PATH", "hermes", "AGENTA_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("AGENTA_GEMINI_PATH", "gemini", "AGENTA_GEMINI_MODEL"); ok {
		agents["gemini"] = e
	}
	if e, ok := probe("AGENTA_PI_PATH", "pi", "AGENTA_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("AGENTA_CURSOR_PATH", "cursor-agent", "AGENTA_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("AGENTA_COPILOT_PATH", "copilot", "AGENTA_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("AGENTA_KIMI_PATH", "kimi", "AGENTA_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("AGENTA_KIRO_PATH", "kiro-cli", "AGENTA_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if e, ok := probe("AGENTA_CODEBUDDY_PATH", "codebuddy", "AGENTA_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	// agy 1.0.6 added a `--model` flag (MUL-3125), so Antigravity now takes a
	// model env like every other backend. AGENTA_ANTIGRAVITY_MODEL seeds the
	...
	if e, ok := probe("AGENTA_ANTIGRAVITY_PATH", "agy", "AGENTA_ANTIGRAVITY_MODEL"); ok {
	...
	claudeArgs, err := shellArgsFromEnv("AGENTA_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("AGENTA_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("AGENTA_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("AGENTA_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("AGENTA_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("AGENTA_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("AGENTA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// AGENTA_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("AGENTA_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// AGENTA_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("AGENTA_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("AGENTA_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via AGENTA_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("AGENTA_DAEMON_ID"))
	...
	// — e.g. ~/.agenta/daemon.id (default) plus ~/.agenta/profiles/<x>/
	...
	// the user explicitly pins AGENTA_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("AGENTA_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("AGENTA_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/AGENTA_workspaces or ~/AGENTA_workspaces_<profile>)
	...
	keepEnv := os.Getenv("AGENTA_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("AGENTA_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("AGENTA_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("AGENTA_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("AGENTA_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("AGENTA_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("AGENTA_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("AGENTA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on agenta Cloud (api.agenta.ai) and opt-out for
	...
	// Operators on either side can flip the default with AGENTA_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("AGENTA_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("AGENTA_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of agenta's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.agenta.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.agenta.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at agenta's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid AGENTA_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("AGENTA_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// AGENTA_WORKSPACES_ROOT env > default ($HOME/AGENTA_workspaces, or
// $HOME/AGENTA_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `agenta daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("AGENTA_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set AGENTA_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "agenta_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "agenta_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("AGENTA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// before any AGENTA_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom AGENTA_*_PATH values never reach this
...
//
//   - The two vars touched (AGENTA_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("AGENTA_OPENCLAW_PATH"); !set {
			_ = os.Setenv("AGENTA_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
