...
// nothing related to agenta before a task ran must contain nothing
// related to agenta after the task finishes — no .agent_context/,
// no .claude/skills/, no .agenta/, no stub directories. The test
...
// must still exist and the agenta-written sibling must be gone.
...
// round-trip to the .agenta/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .agenta/sidecar.txt and created
	// .agenta/ + .agenta/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .agenta/.
	managedDir := filepath.Join(workDir, ".agenta")
	...
	// .agenta still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-agenta/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug agenta would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a agenta skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the agenta
// sibling (which lives at `<slug>-agenta`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Agenta's version",
						Content:     "---\nname: issue-review\n---\n\nAgenta skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a agenta-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.agenta/project/
// resources.json` — the other agenta-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".agenta", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".agenta", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-agenta`.
	...
	if slug != "issue-review-agenta" {
		t.Errorf("second allocation should bump to `-agenta`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-agenta") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-agenta"), 0o755); err != nil {
	...
	if slug != "issue-review-agenta-2" {
		t.Errorf("third allocation should be `-agenta-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-agenta-2") {
...
// `Issue Review` skill, the agenta sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// agenta sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the agenta skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Agenta's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	agentaDir := filepath.Join(workDir, ".claude", "skills", "issue-review-agenta")
	if _, err := os.Stat(filepath.Join(agentaDir, "SKILL.md")); err != nil {
		t.Errorf("Agenta sibling skill should exist at %s: %v", agentaDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// agenta sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(agentaDir); !os.IsNotExist(err) {
		t.Errorf("Agenta sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
