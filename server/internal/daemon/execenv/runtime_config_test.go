	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`agenta issue status <child-id> todo`",
				"all `--status todo`",
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"agenta issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`agenta issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `agenta issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`agenta issue status <this-issue-id> in_review`") {
...
func TestCommentTriggeredBriefCarriesNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "agenta issue comment list "+issueID+" --since "+since+" --output json") {
		t.Errorf("comment brief must keep the issue-wide --since catch-up fallback, got:\n%s", out)
	}
	// The removed resolve step must not reappear.
	if strings.Contains(out, "agenta comment resolve") {
...
func TestCommentTriggeredBriefColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "agenta issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
...
func TestAssignmentTriggeredProtocolHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Run `agenta issue status " + issueID + " in_progress` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		...
		"When done, run `agenta issue status " + issueID + " in_review` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		"If blocked, run `agenta issue status " + issueID + " blocked` unless your Agent Identity forbids issue status changes.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `agenta issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `agenta issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# Agenta Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New Agenta brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier agenta run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what Agenta writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
