...
// agentaManagedBeginMarker / agentaManagedEndMarker delimit the block the
...
const (
	agentaManagedBeginMarker = "# BEGIN agenta-managed (do not edit; regenerated by daemon)"
	agentaManagedEndMarker   = "# END agenta-managed"
)


func renderAgentaManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(agentaManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(agentaManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(agentaManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(agentaManagedEndMarker) + `\n*`)


func upsertAgentaManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderAgentaManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the agenta-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertAgentaManagedBlock(existing, policy)
