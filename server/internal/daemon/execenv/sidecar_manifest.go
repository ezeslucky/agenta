...
const sidecarManifestFile = ".agenta_sidecar_manifest.json"
...
//     so the agent still discovers the agenta skill, just under a
//     different directory name.
//   - For agenta-only namespaces (.agent_context/issue_context.md,
//     .agenta/project/resources.json) the caller swallows the error
...
// down a agenta skill without colliding with a user-installed skill
...
// collision we append `-agenta`, then `-agenta-2`, `-agenta-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + "-agenta"
		default:
			candidate = fmt.Sprintf("%s-agenta-%d", baseSlug, i)
		}
...
// (.agent_context/, .agenta/, .claude/skills/, .github/skills/,
...
// issue-review-agenta. A managed skill directory is platform-owned — the
