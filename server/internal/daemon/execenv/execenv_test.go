...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/ezeslucky/agenta","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".agenta", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"Agent UX 2026",
		"GitHub repo",
		"https://github.com/ezeslucky/agenta",
		"default branch: `main`",
		".agenta/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".agenta", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agenta autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `agenta issue get") {
...
// collision-free duplicates (issue-review, issue-review-agenta,
// issue-review-agenta-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -agenta duplicates", names)
	}
...
// next refresh dodges to issue-review-agenta. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -agenta duplicate", names)
	}
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agenta issue create --title",
		"agenta issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"agenta repo checkout <url>",
		"agenta issue status <id> <status>",
		"agenta issue comment add <issue-id>",
	...
	for _, banned := range []string{
		"agenta issue list [--status",
		"agenta issue label list",
		"agenta issue subscriber list",
		"agenta label list",
		"agenta workspace member list",
		"agenta agent list",
		"agenta squad list",
		"agenta issue runs",
		"agenta issue run-messages",
		"agenta attachment download",
		"agenta autopilot list",
		"agenta autopilot create",
		"agenta autopilot update",
		"agenta autopilot trigger",
		"agenta autopilot delete",
		"agenta project get",
		"agenta project resource list",
		"agenta issue assign",
		"agenta issue label add",
		"agenta issue label remove",
		"agenta issue subscriber add",
		"agenta issue subscriber remove",
		"agenta issue comment delete",
		"agenta label create",
	} {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Agenta Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "agenta issue get") {
		t.Error("should reference agenta CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `agenta issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"agenta issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `agenta issue comment add`",
...
// heredoc on `agenta issue create`, the flags were silently swallowed into
...
// non-ASCII as `?` before reaching `agenta.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		agentaManagedBeginMarker,
		agentaMultiAgentBeginMarker,
		agentaMemoryFeatureBeginMarker,
		agentaMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, agentaManagedBeginMarker) || !strings.Contains(s, agentaManagedEndMarker) {
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), agentaManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.agenta.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, agentaManagedBeginMarker)
	endIdx := strings.Index(s, agentaManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.agenta]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.agenta]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.agenta content")
	}
	...
	if n := strings.Count(string(data2), agentaManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.agenta], got:\n%s", s)
	}
	if strings.Count(s, agentaManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), agentaManagedBeginMarker) {
		t.Error("reused config.toml missing agenta-managed block")
	}
}
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agenta issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "agenta issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigCommentTriggerResumedNoDeltaRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"agenta issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"agenta issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"agenta issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"agenta issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `agenta issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"agenta issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"agenta issue metadata set",
				"agenta issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "agenta issue metadata list issue-md-codex --output json") {
