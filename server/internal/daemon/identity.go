...
// same machine shares one UUID stored at `~/.agenta/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.agenta/profiles/*/daemon.id` and returns every
...
// explicitly pins AGENTA_DAEMON_ID to the hostname itself, there's nothing
