...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (agenta issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"agenta issue create --output json",
		"JSON response",
		"identifier",
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "agenta issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --output json") {
