...
// for the agenta Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: agenta-agent <github@agenta.ai>"
...
// path: a bare cache that already carries the agenta prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: agenta-agent") {
...
// prepare-commit-msg hook that does NOT include the agentaHookMarker
// sentinel — only the older `# Installed by the agenta daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no agentaHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: agenta-agent") {
...
// without the agenta marker (e.g. one a user added manually) must be left
