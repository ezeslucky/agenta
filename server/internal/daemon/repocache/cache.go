...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/AGENTA_workspaces/.repos)
	logger *slog.Logger
...
// agentaHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const agentaHookMarker = "# agenta:prepare-commit-msg:co-authored-by"
...
// treats a hook as agenta-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before agentaHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	agentaHookMarker,
	"# Installed by the Agenta daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Agenta Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# agenta:prepare-commit-msg:co-authored-by
# Agenta: add Co-authored-by trailer for the Agenta Agent.
# Installed by the Agenta daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: agenta-agent <github@agenta.ai>"
...
// a Co-authored-by trailer for the agenta Agent. The hook is installed in the
...
// installed by the agenta daemon (current or any previously released
