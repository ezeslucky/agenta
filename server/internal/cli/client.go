...
// Set by the agenta binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the agenta server API.
...
// reading a capped slice of the body. Every agenta API helper funnels its
...
// It can be overridden with the AGENTA_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring AGENTA_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("AGENTA_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors AGENTA_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects AGENTA_HTTP_TIMEOUT.
