...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `agenta_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("agenta-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("agenta_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/ezeslucky/agenta/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the agenta GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/ezeslucky/agenta/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running agenta binary was installed via Homebrew.
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	tmpFile, err := os.CreateTemp(dir, "agenta-update-*")
	if err != nil {
