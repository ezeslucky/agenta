...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	AgentaUserID  pgtype.UUID // the installer's agenta account
...
// identity is the supplied agentaUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT agenta user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, agentaUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
	...
	if err != nil {
		...
		// lark_user_binding_member_fk (workspace_id, AGENTA_user_id)
...
// (installation_id, lark_open_id) when AGENTA_user_id matches (the
...
// open_id) pair and points at a different agenta user. Account
