...
func TestDispatcher_PlainMessageEnqueuesTask(t *testing.T) {
	...
	if chat.lastEnsureParams.Sender != queries.userBinding.AgentaUserID {
	...
	if enq.lastInitiatorUID != queries.userBinding.AgentaUserID {
		t.Fatalf("p2p task initiator should be sender; got %+v want %+v",
			enq.lastInitiatorUID, queries.userBinding.AgentaUserID)
	}
}
...
// passes the sender's agentaUserID to EnqueueChatTask; this asserts that the
...
// invariant: in a Lark group with TWO agenta bots installed, the
...
func TestDispatcher_LatestSenderWinsAsInitiator(t *testing.T) {
	sessionID := validUUID(0x66)
	alice := boundUser() // agentaUserID 0x55, open id ou_alice below
	bob := boundUser()
	bob.AgentaUserID = validUUID(0xBB)
	queries := &fakeQueries{
		installationByApp:   activeInstallation(),
		chatSession:         db.ChatSession{ID: sessionID, AgentID: validUUID(0x33)},
	...
	if enq.lastInitiatorUID != bob.AgentaUserID {
		t.Fatalf("latest sender (Bob) should be the initiator; got %+v want %+v",
			enq.lastInitiatorUID, bob.AgentaUserID)
	}
}
