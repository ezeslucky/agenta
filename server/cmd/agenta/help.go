...
var rootHelpTemplate = `Work seamlessly with Agenta from the command line.

USAGE
  agenta <command> <subcommand> [flags]
{{range .Groups}}
{{.Title}}
...
EXAMPLES
  ...
  $ agenta daemon start
  $ agenta agent list --output json

ENVIRONMENT VARIABLES
  AGENTA_SERVER_URL    Override the default server URL
  AGENTA_WORKSPACE_ID  Set the active workspace

LEARN MORE
  Use ` + "`agenta <command> <subcommand> --help`" + ` for more information about a command.
`
