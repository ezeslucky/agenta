...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, AGENTA_SERVER_URL env, or 'agenta config set server_url <url>'")
	}
	if inAgentExecutionContext() && !strings.HasPrefix(token, "mat_") {
		return nil, fmt.Errorf("agent execution context requires AGENTA_TOKEN to be a task-scoped mat_ token")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("AGENTA_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("AGENTA_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "AGENTA_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'agenta setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func inAgentExecutionContext() bool {
	return os.Getenv("AGENTA_AGENT_ID") != "" || os.Getenv("AGENTA_TASK_ID") != ""
}

func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "AGENTA_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inAgentExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: AGENTA_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set AGENTA_WORKSPACE_ID env, or run 'agenta config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `agenta agent env set <id>`)")
	}
