...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		t.Setenv("AGENTA_AGENT_ID", "")
		t.Setenv("AGENTA_TASK_ID", "")
		t.Setenv("AGENTA_WORKSPACE_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("AGENTA_AGENT_ID", "agent-123")
		t.Setenv("AGENTA_TASK_ID", "task-456")
		t.Setenv("AGENTA_WORKSPACE_ID", "env-ws")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without AGENTA_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		t.Setenv("AGENTA_AGENT_ID", "agent-123")
		t.Setenv("AGENTA_TASK_ID", "task-456")
		t.Setenv("AGENTA_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want AGENTA_TOKEN", got)
		}
	})
}

func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("AGENTA_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("AGENTA_WORKSPACE_ID", "workspace-123")
	t.Setenv("AGENTA_AGENT_ID", "agent-123")
	t.Setenv("AGENTA_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("AGENTA_TOKEN", "mul_member_token")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("AGENTA_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("AGENTA_WORKSPACE_ID", "test-ws")
	t.Setenv("AGENTA_TOKEN", "test-token")
	t.Setenv("AGENTA_AGENT_ID", "")
	t.Setenv("AGENTA_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "agenta agent env set") {
		t.Fatalf("no-fields error must direct users to `agenta agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `agenta agent env set` instead", flag)
		}
	}
}
...
func TestAgentSkillsAddCallsAdditiveEndpoint(t *testing.T) {
	...
	t.Setenv("AGENTA_SERVER_URL", srv.URL)
	t.Setenv("AGENTA_WORKSPACE_ID", "ws-1")
	t.Setenv("AGENTA_TOKEN", "test-token")
