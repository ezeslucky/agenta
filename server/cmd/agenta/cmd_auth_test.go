...
func TestMain(m *testing.M) {
	os.Unsetenv("AGENTA_AGENT_ID")
	os.Unsetenv("AGENTA_TASK_ID")
	os.Unsetenv("AGENTA_TOKEN")
	os.Exit(m.Run())
}
...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers AGENTA_APP_URL", func(t *testing.T) {
		t.Setenv("AGENTA_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `agenta login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
