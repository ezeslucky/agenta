...
func TestPrintDiskUsageEmptyHintSuggestsProfilesWithTasks(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("AGENTA_WORKSPACES_ROOT", "")
	...
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "agenta_workspaces"),
	}, "", "")
	...
	if !strings.Contains(got, "agenta --profile two-tasks daemon disk-usage") {
		t.Fatalf("hint output = %q, want two-tasks profile command", got)
	}
	if !strings.Contains(got, "agenta --profile one-task daemon disk-usage") {
		t.Fatalf("hint output = %q, want one-task profile command", got)
	}
	if !strings.Contains(got, "agenta --profile 'space profile' daemon disk-usage") {
...
func TestPrintDiskUsageEmptyHintSuggestsDefaultFromNamedProfile(t *testing.T) {
	...
	var out bytes.Buffer
	printDiskUsageEmptyHint(&out, daemon.DiskUsageReport{
		WorkspacesRoot: filepath.Join(home, "agenta_workspaces_named"),
	}, "named", "")

	got := out.String()
	if !strings.Contains(got, "agenta daemon disk-usage") {
...
func mkdirProfile(t *testing.T, home, profile string) {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(home, ".agenta", "profiles", profile), 0o755); err != nil {
		t.Fatal(err)
	}
}
...
func writeDiskUsageTaskFile(t *testing.T, home, profile, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "agenta_workspaces_"+profile, workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}

func writeDefaultDiskUsageTaskFile(t *testing.T, home, workspaceID, taskID, rel string) {
	t.Helper()
	path := filepath.Join(home, "agenta_workspaces", workspaceID, taskID, rel)
	writeDiskUsageFile(t, path)
}
