...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("AGENTA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("AGENTA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("AGENTA_WORKSPACE_ID")
	agentName := os.Getenv("AGENTA_AGENT_NAME")
	taskID := os.Getenv("AGENTA_TASK_ID")
