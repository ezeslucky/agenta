...
var rootCmd = &cobra.Command{
	Use:           "agenta",
	Short:         "Agenta CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Agenta from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("agenta {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Agenta server URL (env: AGENTA_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: AGENTA_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: AGENTA_DEBUG)")
