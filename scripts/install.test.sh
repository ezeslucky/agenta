...
_setup_sandbox() {
  ...
  cat >"$payload_dir/agenta" <<'STUB'
#!/usr/bin/env bash
echo "agenta v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/agenta"
  tar -czf "$tmp/agenta.tar.gz" -C "$payload_dir" agenta

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/ezeslucky/agenta/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    AGENTA_BIN_DIR="$tmp/install-bin" \
    AGENTA_TEST_ARCHIVE="$tmp/agenta.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/agenta" ]]; then
    echo "expected fallback binary at $tmp/install-bin/agenta" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
