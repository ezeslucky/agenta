...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/ezeslucky/agenta.git"
$RepoWebUrl    = "https://github.com/ezeslucky/agenta"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".agenta\server"
$InstallDir    = if ($env:AGENTA_INSTALL_DIR) { $env:AGENTA_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/ezeslucky/agenta/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:AGENTA_SELFHOST_REF) {
        return $env:AGENTA_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = agenta version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Agenta CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Agenta requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/ezeslucky/agenta/releases/download/$latest/agenta-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "agenta-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "agenta.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/ezeslucky/agenta/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "agenta.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "agenta-cli-$version-windows-$arch.zip"
        $legacyAsset = "agenta_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "agenta.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".agenta\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "agenta.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "agenta.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "agenta.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "agenta.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Agenta CLI installed to $binDir\agenta.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "agenta") {
        ...
        if ($isUpToDate) {
            Write-Ok "Agenta CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Agenta CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Agenta CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "agenta")) {
        Write-Fail "CLI installed but 'agenta' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Agenta self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:AGENTA_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Agenta server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Agenta repository..."
    ...
    Write-Info "Pulling official Agenta images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Agenta services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Agenta server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Agenta - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Agenta CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     agenta setup               " -NoNewline; Write-Host "# Connect to Agenta Cloud (agenta.ai)" -ForegroundColor DarkGray
    Write-Host "     agenta setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:AGENTA_MODE="with-server"; irm https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Agenta - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Agenta server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     agenta setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:AGENTA_MODE="stop"; irm https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Agenta services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Agenta installation found at $InstallDir"
    }

    if (Test-CommandExists "agenta") {
        try {
            agenta daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:AGENTA_MODE) { $env:AGENTA_MODE.ToLower() } else { "default" }
