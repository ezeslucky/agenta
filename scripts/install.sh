...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Agenta supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Agenta CLI already installed via Homebrew"
    else
      warn "Failed to install agenta via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Agenta CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Agenta CLI from GitHub Releases..."
  ...
  local url="https://github.com/ezeslucky/agenta/releases/download/${latest}/agenta-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/agenta.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/agenta.tar.gz" -C "$tmp_dir" agenta

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with AGENTA_BIN_DIR.
  local bin_dir="${AGENTA_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/agenta" "$bin_dir/agenta"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/agenta" "$bin_dir/agenta"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/agenta" "$bin_dir/agenta"
    chmod +x "$bin_dir/agenta"
  ...
  rm -rf "$tmp_dir"
  ok "Agenta CLI installed to $bin_dir/agenta"
}
...
get_selfhost_ref() {
  if [ -n "${AGENTA_SELFHOST_REF:-}" ]; then
    printf '%s' "$AGENTA_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Agenta CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Agenta CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Agenta CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists agenta; then
    local current_ver
    # `agenta version` outputs "agenta 0.3.23 (commit: f46b929eb, built: 2026-06-16T10:11:56Z)" — extract just the version
    current_ver=$(agenta version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Agenta CLI is up to date ($current_ver)"
      return 0
    fi

    info "Agenta CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(agenta version 2>/dev/null | awk 'NR==1{print $2}' || echo "unknown")
    ok "Agenta CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists agenta; then
    fail "CLI installed but 'agenta' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Agenta self-hosting requires Docker and Docker Compose.
...
setup_server() {
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning Agenta repository..."
  ...
  info "Pulling official Agenta images..."
  pull_official_selfhost_images
  info "Starting Agenta services (this may take a few minutes on first run)..."
...
run_default() {
  printf "\n"
  printf "${BOLD}  Agenta — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Agenta CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}agenta setup${RESET}                # Connect to Agenta Cloud (agenta.ai)\n"
  printf "     ${CYAN}agenta setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Agenta — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Agenta server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}agenta setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Agenta installation found at $INSTALL_DIR"
  fi

  if command_exists agenta; then
    agenta daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Agenta CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  AGENTA_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.agenta/server)"
        echo "  AGENTA_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  AGENTA_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'agenta setup' to configure your environment."
        exit 0
        ;;
