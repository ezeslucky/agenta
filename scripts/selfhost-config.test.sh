...
require_config "$config" 'AGENTA_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "AGENTA_APP_URL=${AGENTA_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "AGENTA_SERVER_URL=${AGENTA_SERVER_URL}" \
...
require_env "$local_env" 'AGENTA_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'AGENTA_SERVER_URL=ws://localhost:9100/ws'
