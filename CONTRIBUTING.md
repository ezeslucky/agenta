...
This guide documents the local development workflow for contributors working on the Agenta codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=agenta`
...
POSTGRES_DB=agenta
POSTGRES_PORT=5432
DATABASE_URL=postgres://agenta:agenta@localhost:5432/agenta?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=agenta_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://agenta:agenta@localhost:5432/agenta_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../agenta-feature -b feat/my-change main
cd ../agenta-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `agenta_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`agenta login`).
...
whatever server is configured in `~/.agenta/config.json`. That's fine for
...
Example: worktree at `../agenta-feat-auth` produces profile
`dev-agenta_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `AGENTA_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.agenta/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `agenta` commands automatically use the same
...
rm -rf "$HOME/.agenta/profiles/$PROFILE"
```
...
1. Compiles the `agenta` CLI from `server/cmd/agenta` into
   `apps/desktop/resources/bin/agenta`
...
backend logs. If you set `AGENTA_DEV_VERIFICATION_CODE=888888` before starting
...
Nothing in this flow touches the system-installed `agenta` or the default
`~/.agenta/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.agenta/config.json` | `~/.agenta/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.agenta/daemon.pid` | `~/.agenta/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/agenta_workspaces/` | `~/agenta_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `agenta_<slug>_<hash>` |
| Desktop profile | `desktop-api.agenta.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U agenta -d postgres -At -c "select datname from pg_database order by datname;"
```
