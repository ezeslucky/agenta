...
export class ApiClient {
  ...
  private readCsrfToken(): string | null {
    ...
    const match = document.cookie
      .split("; ")
      .find((c) => c.startsWith("agenta_csrf="));
  ...
  // ---------------------------------------------------------------------
  // Cloud Billing — proxies to agenta-cloud /api/v1/billing/*. The
  // agenta-api server stamps X-User-ID and forwards bytes; everything
