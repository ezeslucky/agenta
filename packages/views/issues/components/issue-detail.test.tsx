...
import type { Issue, TimelineEntry } from "@agenta/core/types";
...
vi.mock("@agenta/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @agenta/core/hooks.tsx). vi.mock("@agenta/core/paths") only intercepts
...
// Mock @agenta/core/workspace/hooks
vi.mock("@agenta/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @agenta/core/paths — after the URL-driven workspace refactor,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.agenta.com${p}`,
  }),
...
// Mock issue config
vi.mock("@agenta/core/issues/config", () => ({
...
vi.mock("@agenta/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @agenta/ui/components/ui/resizable)
