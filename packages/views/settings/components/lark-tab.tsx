...
import { larkInstallationsOptions, larkKeys } from "@agenta/core/lark";
import { api, ApiError } from "@agenta/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@agenta/core/types";
...
// install path is per-agent (each Agenta Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Agenta Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Agenta agent's
