...
import {
  ...
  InputOTPGroup,
  InputOTPSlot,
} from "@agenta/ui/components/ui/input-otp";
...
import type { User } from "@agenta/core/types";
    ...
    api
      ...
      .catch(() => {
        ...
        api
          ...
          .catch(() => {
            api.setToken(null);
            localStorage.removeItem("agenta_token");
          });
      });
  }, [cliCallback]);
    ...
    async (value: string) => {
      ...
      try {
        if (cliCallback) {
          ...
          localStorage.setItem("agenta_token", token);
          api.setToken(token);
          onTokenObtained?.();
  ...
  const handleCliAuthorize = async () => {
    ...
    try {
      ...
      if (authSourceRef.current === "localStorage") {
        ...
        const stored = localStorage.getItem("agenta_token");
