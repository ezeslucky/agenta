...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Agenta' title", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to agenta/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("agenta_token", "existing-jwt");
