...
vi.mock("@agenta/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@agenta/core/onboarding")>(
      "@agenta/core/onboarding",
    );
  return { ...actual, saveQuestionnaire: mockSaveQuestionnaire };
});

vi.mock("@agenta/core/analytics", () => ({
...
function wipeDismissCounters() {
  for (let i = window.localStorage.length - 1; i >= 0; i--) {
    ...
    if (k && k.startsWith("agenta.source_backfill.dismiss.")) {
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Agenta/i),
  ...
  it("opens for an onboarded user with empty source and fires the shown event", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Agenta/i),
      ).toBeInTheDocument();
    });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("agenta.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
