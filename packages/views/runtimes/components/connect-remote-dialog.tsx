...
import { runtimeKeys } from "@agenta/core/runtimes/queries";
import { useWSEvent } from "@agenta/core/realtime";
import { paths, useWorkspaceSlug } from "@agenta/core/paths";
...
import { CODE_LIGATURE_CLASS } from "@agenta/ui/lib/code-style";
...
const INSTALL_CMD =
  "curl -fsSL https://raw.githubusercontent.com/ezeslucky/agenta/main/scripts/install.sh | bash";
const CLOUD_SERVER_URL = "https://api.agenta.ai";
const CLOUD_APP_URL = "https://agenta.ai";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `agenta setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `agenta config set server_url ${normalizedServerUrl}
agenta config set app_url ${normalizedAppUrl}
agenta login --token <YOUR_TOKEN>
agenta daemon start`,
    };
  }

  return {
    setupCmd: "agenta setup",
    tokenCmd: `agenta config set server_url ${CLOUD_SERVER_URL}
agenta config set app_url ${CLOUD_APP_URL}
agenta login --token <YOUR_TOKEN>
agenta daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `agenta setup` is one blocking command that handles config + login
