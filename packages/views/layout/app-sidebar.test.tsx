...
import { ApiError } from "@agenta/core/api";
...
vi.mock("@agenta/ui/components/ui/sidebar", () => ({
...
vi.mock("@agenta/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@agenta/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));

vi.mock("@agenta/core/auth", () => ({
  useAuthStore: (selector: (state: { user: { id: string } }) => unknown) => selector({ user: { id: "user-1" } }),
}));
vi.mock("@agenta/core/paths", () => ({
...
vi.mock("@agenta/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@agenta/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@agenta/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@agenta/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@agenta/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@agenta/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@agenta/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@agenta/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@agenta/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@agenta/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@agenta/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
vi.mock("@agenta/core/workspace/queries", () => ({
