...
// agenta CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // agenta://auth/callback?token=<jwt>
...
function createWindow(): void {
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      plugins: true,
      additionalArguments: [`--agenta-locale=${systemLocale}`],
    },
  });
...
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Agenta Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Agenta Canary";
...
if (is.dev) {
  ...
  // to "agenta", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.agenta.desktop.dev" : "ai.agenta.desktop",
    );
