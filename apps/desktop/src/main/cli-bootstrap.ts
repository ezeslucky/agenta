...
// Desktop prefers the bundled `agenta` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/ezeslucky/agenta/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "agenta.exe" : "agenta";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `agenta-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `agenta` binary. If one is already present at
