const RELEASE_ARCHIVE_PREFIX = "agenta-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `agenta-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `agenta_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `agenta_${os}_${mappedArch}.${ext}`;
