...
const PREFS_PATH = join(homedir(), ".agenta", "desktop_prefs.json");
...
// `agenta daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/agenta/cmd_daemon.go) to
...
// Keep the Go impl in sync: server/cmd/agenta/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".agenta", "profiles", profile)
    : join(homedir(), ".agenta");
}
 ...
 * a stuck start is an auth problem. Hits the same endpoint `agenta auth status`
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["agenta.exe"] : ["agenta"];
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "agenta.exe" : "agenta";
...
/**
 * Returns a usable `agenta` binary path. Priority:
 ...
 *   5. `agenta` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Agenta Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, AGENTA_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "agenta CLI is not installed" };
