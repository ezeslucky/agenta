...
import { CoreProvider } from "@agenta/core/platform";
import { pickLocale, type SupportedLocale } from "@agenta/core/i18n";
import { useAuthStore } from "@agenta/core/auth";
import { useWelcomeStore } from "@agenta/core/onboarding";
import { workspaceKeys, workspaceListOptions } from "@agenta/core/workspace/queries";
import { api } from "@agenta/core/api";
import { useHasOnboarded } from "@agenta/core/paths";
import { setCurrentWorkspace } from "@agenta/core/platform";
import { ThemeProvider } from "@agenta/ui/components/common/theme-provider";
import { AgentaIcon } from "@agenta/ui/components/common/agenta-icon";
import { Toaster } from "@agenta/ui/components/ui/sonner";
...
import { captureEvent } from "@agenta/core/analytics";
import { RESOURCES } from "@agenta/views/locales";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.agenta config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (agenta://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (agenta://auth/callback?token=...).
  ...
  useEffect(() => {
    if (!user) return;
    const token = localStorage.getItem("agenta_token");
    if (!token) return;
  ...
  if (isLoading || bootstrapping) {
    return (
      <div className="flex h-screen items-center justify-center">
        <AgentaIcon className="size-6 animate-pulse" />
      </div>
    );
  }
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Agenta Desktop could not load <code>~/.agenta/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
