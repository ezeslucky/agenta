...
import { ModalRegistry } from "@agenta/views/modals/registry";
import { AppSidebar } from "@agenta/views/layout";
import { SearchCommand, SearchTrigger } from "@agenta/views/search";
import { ChatFab, ChatWindow } from "@agenta/views/chat";
import { WorkspaceSlugProvider, paths, useCurrentWorkspace } from "@agenta/core/paths";
import { useNavigation } from "@agenta/views/navigation";
import { getCurrentSlug, subscribeToCurrentSlug } from "@agenta/core/platform";
import { useDesktopUnreadBadge } from "@agenta/views/platform";
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("agenta:navigate", handler);
    return () => window.removeEventListener("agenta:navigate", handler);
  }, []);
}
 ...
 * `agenta:navigate` event, whose handler `openTab`s into the ACTIVE
