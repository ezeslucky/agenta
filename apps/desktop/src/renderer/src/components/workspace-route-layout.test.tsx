...
vi.mock("@agenta/core/platform", () => ({
  setCurrentWorkspace: vi.fn(),
}));

vi.mock("@agenta/core/workspace", async () => {
  const actual = await vi.importActual<typeof import("@agenta/core/workspace")>(
    "@agenta/core/workspace",
  );
  return {
    ...actual,
...
vi.mock("@agenta/views/workspace/use-workspace-seen", () => ({
  useWorkspaceSeen: () => state.workspaceSeen,
}));

vi.mock("@agenta/views/workspace/welcome-after-onboarding", () => ({
  WelcomeAfterOnboarding: () => null,
}));

vi.mock("@agenta/views/layout", () => ({
...
vi.mock("@agenta/views/onboarding", () => ({
  SourceBackfillModal: () => {
