...
import enCommon from "@agenta/views/locales/en/common.json";
import enAuth from "@agenta/views/locales/en/auth.json";
import enSettings from "@agenta/views/locales/en/settings.json";
...
vi.mock("@agenta/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@agenta/core/auth")>(
      "@agenta/core/auth",
    );
...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to Agenta")).toBeInTheDocument();
  ...
  it("calls sendCode with email on submit", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "test@agenta.ai");
    await user.click(screen.getByRole("button", { name: "Continue" }));

    await waitFor(() => {
      expect(mockSendCode).toHaveBeenCalledWith("test@agenta.ai");
    });
  });
  ...
  // the Agenta:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@agenta.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "agenta://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open Agenta Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
