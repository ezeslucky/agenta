...
import { sanitizeNextUrl, useAuthStore } from "@agenta/core/auth";
...
import {
  ...
  CardDescription,
  CardContent,
} from "@agenta/ui/components/ui/card";
import { Button } from "@agenta/ui/components/ui/button";
import { Loader2 } from "lucide-react";
import { captureDownloadIntent } from "@agenta/core/analytics";
import { setLoggedInCookie } from "@/features/auth/auth-cookie";
import Link from "next/link";
import { LoginPage, validateCliCallback } from "@agenta/views/auth";
import { useT } from "@agenta/views/i18n";
    ...
    if (isDesktopHandoff) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `agenta://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
          setDesktopError(
          ...
          <CardContent className="flex justify-center">
            {desktopToken ? (
              <Button
                variant="outline"
                onClick={() => {
                  window.location.href = `agenta://auth/callback?token=${encodeURIComponent(desktopToken)}`;
                }}
              >
