...
import { LOCALE_COOKIE } from "@agenta/core/i18n";
import {
  AGENTA_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-agenta-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(AGENTA_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("agenta_logged_in");
