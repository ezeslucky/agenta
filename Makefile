.PHONY: help makehelp dev server daemon cli agenta build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@echo "==> Pulling official Agenta images..."
	@if ! docker compose -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${AGENTA_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Agenta via Docker Compose..."
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Agenta from the current checkout..."
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) agenta AGENTA_ARGS="daemon restart --profile local"

cli: ## Run the agenta CLI with ARGS or AGENTA_ARGS from source
	@$(MAKE) agenta AGENTA_ARGS="$(AGENTA_ARGS)"

agenta: ## Run the agenta CLI entrypoint directly from the Go source tree
	cd server && go run ./cmd/agenta $(AGENTA_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/agenta ./cmd/agenta
