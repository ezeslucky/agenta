...
test("onboarding v2 — welcome → source → role → use_case (skip path)", async ({ page }) => {
  ...
  await page.addInitScript((t) => {
    localStorage.setItem("agenta_token", t);
  }, token);
  ...
  // 2. Source step
  await expect(page.getByText("How did you hear about Agenta?")).toBeVisible({ timeout: 10000 });
  ...
  // 4. Use case step
  await expect(page.getByText("What do you want to use Agenta for?")).toBeVisible({ timeout: 10000 });
...
test("onboarding v2 — rage-skip all 3 questions", async ({ page }) => {
  ...
  await page.addInitScript((t) => localStorage.setItem("agenta_token", t), token);
  await page.goto("/onboarding", { waitUntil: "domcontentloaded" });
  await waitForPageText(page, "Continue on web");
...
test("onboarding v2 — zh-Hans renders Chinese labels", async ({ page, context }) => {
  await context.addCookies([
    { name: "agenta-locale", value: "zh-Hans", url: "http://localhost:13442" },
  ]);
  const api = new TestApiClient();
  ...
  await expect(page.getByText("你是从哪里了解到 Agenta 的？")).toBeVisible({ timeout: 10000 });
  await page.waitForTimeout(500);
