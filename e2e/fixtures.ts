...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://agenta:agenta@localhost:5432/agenta?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.AGENTA_DEV_VERIFICATION_CODE?.trim();
